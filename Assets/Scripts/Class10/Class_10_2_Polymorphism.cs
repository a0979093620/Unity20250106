...
namespace lotta_10
{
    public class Class_10_2_Polymorphism : MonoBehaviour
    {
        private void Awake()
        {
            Trap trap = new Trap("落穴", 3);
            Magic magic = new Magic("死者甦醒", 5);

            // 多型 : 多種形式，可以使用父類別或自己的成員
            trap.Information();         // 型別 1 : 呼叫父類別的方法
            trap.Information(37);       // 型別 2 : 呼叫自己有一個參數的方法
            ...
            magic2.Information();       // 形式 2 : 呼叫 Magic 的方法
        }
    ...
    public class Trap : Card
    {
        ...
        public void Information(int _index)
        {
            LogSystem.LogWithColor($"這是一張陷阱卡，編號: {_index}", "#f77");
